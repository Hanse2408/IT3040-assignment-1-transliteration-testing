...
def _visible_first(page, selector: str):
    try:
...
def _find_tmrtools_locators(page, timeout_ms: int):
    """Locate the input/output fields on https://tmrtools.com/.
    The site can change small class names, so this uses visible text/placeholder based fallbacks.
    """
    deadline = time.time() + (max(1, timeout_ms) / 1000)
    last_debug = None
    while time.time() < deadline:
        ...
        # Make sure the text input tab is active if the site shows Text / Upload File tabs.
        for tab_name in [r"^Text$", r"Input Source"]:
            try:
                tab = page.get_by_text(re.compile(tab_name, re.IGNORECASE)).first
                if tab.is_visible():
                    try:
        ...
        input_locator = None
        input_selectors = [
            'textarea[placeholder*="Type Singlish"]',
        ...
        for selector in input_selectors:
            input_locator = _visible_first(page, selector)
            if input_locator:
                break
        ...
        output_selectors = [
            '.ql-editor',
            '[data-placeholder*="Translated Sinhala"]',
            '[aria-label*="Sinhala"]',
            'textarea[readonly]',
            'textarea[disabled]',
            '[role="textbox"][contenteditable="true"]',
        ...
        # Prefer the output area after the visible "Sinhala Output" label.
        try:
            after_label = page.locator(
                'text=Sinhala Output >> xpath=following::*[self::textarea or @contenteditable="true" or contains(@class,"ql-editor") or @role="textbox"][1]'
            ).first
            if after_label.count() > 0 and after_label.is_visible():
                output_locator = after_label
        except Exception:
            pass
                ...
                try:
                    ...
                    for i in range(count):
                        ...
                        try:
                            ...
                            pass
                    if selector == 'textarea' and len(visible) >= 2:
                        output_locator = visible[1]
                        break
                    if selector != 'textarea' and visible:
                        # If only one contenteditable exists, it is usually the Sinhala output editor.
                        output_locator = visible[-1]
                        break
                except Exception as e:
                    last_debug = str(e)
        ...
        action_locator = None
        for pattern in [r"^(Translate|Convert|Transliterate)$", r"(Translate|Convert|Transliterate)"]:
            try:
        ...
        if input_locator and output_locator:
        ...
        meta = page.evaluate(
            """() => ({
              textareas: Array.from(document.querySelectorAll('textarea')).map(t => ({
                placeholder: t.getAttribute('placeholder') || '', value: t.value || '',
                disabled: !!t.disabled, readOnly: !!t.readOnly, visible: !!(t.offsetParent)
              })),
              editables: Array.from(document.querySelectorAll('[contenteditable="true"], .ql-editor, [role="textbox"]')).map(e => ({
                tag: e.tagName, text: (e.innerText || e.textContent || '').slice(0, 80),
                cls: e.className || '', visible: !!(e.offsetParent)
              }))
            })"""
        )
        print("Debug: tmrtools UI meta:", meta)
    except Exception as e:
        print("Debug: failed to read tmrtools UI meta:", e)
    if last_debug:
        print("Debug: last error:", last_debug)
    raise RuntimeError("Could not find TMR Tools UI locators (input/output fields).")
...
def _parse_args():
    ...
    parser.add_argument("--browser-channel", default="msedge", help="Browser channel: msedge, chrome, or leave empty for bundled chromium")
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        launch_kwargs = {"headless": args.headless, "slow_mo": max(0, int(args.slow_mo_ms))}
        if args.browser_channel:
            launch_kwargs["channel"] = args.browser_channel
            print(f"Opening browser channel: {args.browser_channel}")
        ...
        is_tmrtools = "tmrtools.com" in (args.url or "")
        if is_chat:
            try:
            ...
            try:
                input_locator, output_locator, action_locator = _find_tmrtools_locators(page, int(args.timeout_ms))
            except Exception as e:
                print(f"Error locating TMR Tools UI elements: {e}")
                browser.close()
                return
        else:
